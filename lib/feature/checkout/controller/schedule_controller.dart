...
class ScheduleController extends GetxController implements GetxService{
  ...
  void onInit() {
    ...
    try {
      ...
      if (sharedPref.containsKey('saved_schedule_time')) {
        scheduleTime = sharedPref.getString('saved_schedule_time');
      }
      if (sharedPref.containsKey('saved_schedule_type')) {
        String? typeStr = sharedPref.getString('saved_schedule_type');
        if (typeStr == 'asap') {
  ...
  void _saveToSharedPreferences() {
    try {
      ...
      if (scheduleTime != null) {
        sharedPref.setString('saved_schedule_time', scheduleTime!);
      } else {
        sharedPref.remove('saved_schedule_time');
      }
      sharedPref.setString('saved_schedule_type', _selectedScheduleType.name);
    } catch (e) {
      //
    }
  }
  ...
  updateScheduleType({bool shouldUpdate = true, required ScheduleType scheduleType}){
    ...
    if(scheduleType == ScheduleType.asap){
      if(_initialSelectedScheduleType != ScheduleType.asap){
      ...
      if(_initialSelectedScheduleType != ScheduleType.schedule){
