...
class AllSearchController extends GetxController implements GetxService {
  ...
  Future<void> navigateToSearchResultScreen()async{
    ...
    if(searchController.value.text.trim().isNotEmpty){
      String queryText = searchController.value.text.trim();
      ...
      if (category != null) {
        String route = RouteHelper.getCategoryServiceFormRoute(
          category.id ?? '',
          category.name ?? '',
          category.imageFullPath ?? '',
        );
        if(Get.currentRoute.contains('/category-service-form')){
          Get.offNamed(route);
        } else {
          Get.toNamed(route);
        }
    ...
    if (Get.find<CategoryController>().categoryList != null) {
      for (var element in Get.find<CategoryController>().categoryList!) {
        if (element.name != null && element.name!.toLowerCase().contains(query.toLowerCase())) {
          _searchSuggestionList.add(SearchSuggestion(name: element.name, isSearched: 0));
        }
      }
    }
