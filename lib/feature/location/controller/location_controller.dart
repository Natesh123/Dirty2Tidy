...
class LocationController extends GetxController implements GetxService {
  ...
  String  countryDialCode = CountryCode.fromCountryCode(Get.find<SplashController>().configModel.content?.countryCode ?? "AU").dialCode!;
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel, bool fromAddAddressScreen) async {
    _isLoading = true;
    update();
    ...
    if (response.body["response_code"] == "default_store_200") {
      await getAddressList();
      AddressModel serverAddress = AddressModel.fromJson(response.body["content"]);
      if(fromAddAddressScreen){
        Get.back();
        if(serverAddress.zoneId == getUserAddress()?.zoneId){
          _selectedAddress = serverAddress;
        ...
        _selectedAddress = serverAddress;
        await saveUserAddress(serverAddress);
      }
      responseModel = ResponseModel(true, response.body["message"]);
    } else {
  ...
  Future<bool> saveUserAddress(AddressModel address) async {
    ...
    _userAddress = address; // Update local cache
  ...
  void setTemporaryZone(String zoneId) {
    _zoneID = zoneId;
    if (_userAddress != null) {
      _userAddress!.zoneId = zoneId;
      locationRepo.saveUserAddress(jsonEncode(_userAddress!.toJson()), zoneId);
    }
    Get.find<ApiClient>().updateHeader(
        Get.find<ApiClient>().token,
        zoneId,
  ...
  AddressModel? getUserAddress() {
    if (_userAddress != null) return _userAddress;

    AddressModel? addressModelUser;
    try {
      if(locationRepo.getUserAddress() == null) return null;
       addressModelUser = AddressModel.fromJson(jsonDecode(locationRepo.getUserAddress()!));
       _userAddress = addressModelUser; // Cache it
  ...
  void autoNavigate(AddressModel address, bool fromSignUp, String? route, bool canRoute, AddressModel? previousAddress, bool? isServiceAvailable, {bool shouldCartDelete = false,  String? showDialog}) async {
    ...
    // HomeScreen.loadData(true); // Redundant and causes dual API storms on Flutter Web
