...
class SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _initData() async {
    ...
    if( Get.find<SplashController>().getGuestId().isEmpty || Get.find<SplashController>().getGuestId().length > 36){
      var uuid = const Uuid().v1();
      await Get.find<SplashController>().setGuestId(uuid);
    }
        ...
        Timer(const Duration(seconds: 1), () async {
          ...
          if(_checkAvailableUpdate()) {
            ...
            if(Get.find<SplashController>().isShowInitialLanguageScreen()){
                ...
                Get.find<SplashController>().updateLanguage(false);
              }
