...
class ConversationController extends GetxController with GetSingleTickerProviderStateMixin implements GetxService{
    ...
    channelScrollController1.addListener(() {
      if (channelScrollController1.position.pixels == channelScrollController1.position.maxScrollExtent){
        if (_channelOffset! < _channelPageSize!) {
          getChannelList(_channelOffset!+1, isFromPagination: true, type: tabController!.index == 0 ? "provider" : "serviceman");
        }
      }
    });
    ...
    if (_providerChannelList != null) {
      for (var channel in _providerChannelList!) {
        ...
        if (channel.channelUsers != null && channel.channelUsers!.length > 1) {
          isRead = channel.channelUsers![0].user?.userType == "customer" ? channel.channelUsers![0].isRead! : channel.channelUsers![1].isRead!;
        }
        if (isRead == 0) {
          count++;
        }
  ...
  int getUnreadServicemanCount() {
    int count = 0;
    if (_servicemanChannelList != null) {
      for (var channel in _servicemanChannelList!) {
        int? isRead;
        if (channel.channelUsers != null && channel.channelUsers!.length > 1) {
          isRead = channel.channelUsers![0].user?.userType == "customer" ? channel.channelUsers![0].isRead! : channel.channelUsers![1].isRead!;
        }
