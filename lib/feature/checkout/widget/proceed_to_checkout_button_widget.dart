...
class ProceedToCheckoutButtonWidget extends StatefulWidget {
  ...
  final double? amount;
  const ProceedToCheckoutButtonWidget({super.key, required this.pageState, required this.addressId, this.amount}) ;
...
class _ProceedToCheckoutButtonWidgetState extends State<ProceedToCheckoutButtonWidget> {
    ...
    return GetBuilder<ScheduleController>(builder: (scheduleController){
      return GetBuilder<CartController>(builder: (cartController){
        ...
        return GetBuilder<CheckOutController>(builder: (checkoutController){
          ...
          double cartGrandTotal = CheckoutHelper.calculateGrandTotal(cartList: cartController.cartList, referralDiscount: cartController.referralAmount, daysCount: scheduleDaysCount, applicableCouponCount: applicableCouponCount);
          double totalAmount = widget.amount ?? (checkoutController.totalAmount > 0 ? checkoutController.totalAmount : cartGrandTotal);
          bool isPartialPayment = CheckoutHelper.checkPartialPayment(walletBalance: cartController.walletBalance, bookingAmount: totalAmount);
                    ...
                    if(Get.find<CartController>().cartList.isEmpty) {
                      ...
                      if(cartController.walletPaymentStatus && isPartialPayment && checkoutController.selectedPaymentMethod == PaymentMethodName.walletMoney){
                        ...
                        checkoutController.placeBookingRequest(
                          ...
                          note: checkoutController.serviceNoteController.text,
                        );
                      }
                        ...
                        if(checkoutController.selectedDigitalPaymentMethod != null && checkoutController.selectedDigitalPaymentMethod?.gateway != "offline"){
                          _makeDigitalPayment(addressModel, checkoutController.selectedDigitalPaymentMethod, isPartialPayment, checkoutController, totalAmount);
                        }else{
  ...
  _makeDigitalPayment(AddressModel? address , DigitalPaymentMethod?  paymentMethod, bool isPartialPayment, CheckOutController checkoutController, double totalAmount) {
    ...
    String? repeatBookingDates = "[]";
    String? bookingType = "regular";

    if(scheduleController.selectedServiceType == ServiceType.repeat){
      bookingType = scheduleController.selectedRepeatBookingType.name;
      repeatBookingDates = CheckoutHelper.getRepeatBookingScheduleList(
        repeatBookingType: scheduleController.selectedRepeatBookingType,
        dateRange: scheduleController.selectedRepeatBookingType == RepeatBookingType.daily ? scheduleController.pickedDailyRepeatBookingDateRange
            : scheduleController.pickedWeeklyRepeatBookingDateRange,
        time: scheduleController.selectedRepeatBookingType == RepeatBookingType.daily ? scheduleController.pickedDailyRepeatTime : scheduleController.pickedWeeklyRepeatTime ,
    ...
    try {
      ...
      if(cartController.cartList.isNotEmpty){
        ...
        Get.find<LeadController>().saveLeadAddress(
          ...
          serviceCost: cartItem.serviceCost.toDouble(),
          taxAmount: cartItem.taxAmount.toDouble(),
          discountAmount: cartItem.discountedPrice.toDouble(),
          couponCode: cartItem.couponCode,
          providerId: cartItem.provider?.id,
          isPaid: false
        );
      }
    } catch (e) {
      if (kDebugMode) {
        print("Lead Update Error: $e");
      }
    }
    ...
    url = '${AppConstants.baseUrl}/payment?payment_method=${paymentMethod?.gateway}&access_token=${base64Url.encode(utf8.encode(userId))}&zone_id=$zoneId'
        ...
        '&payment_platform=$platform&service_location=$serviceLocation&booking_type=$bookingType&dates=$repeatBookingDates';
