...
class PaymentView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return isSubBooking && bookingDetails.isPaid == 0 && (bookingDetails.bookingStatus == "ongoing" || bookingDetails.bookingStatus == "accepted")
        ? _MakePaymentView(bookingDetails, totalBookingAmount: totalBookingAmount)
        : _PaidPaymentView(bookingDetails, totalBookingAmount: totalBookingAmount);
  }
}
...
class _PaidPaymentView extends StatelessWidget {
  ...
  const _PaidPaymentView(this.bookingDetails, {required this.totalBookingAmount});

  @override
  Widget build(BuildContext context) {

    bool isPartialPayment = bookingDetails.partialPayments !=null && bookingDetails.partialPayments!.isNotEmpty;
    double payAmount = totalBookingAmount;
    if(isPartialPayment){
...
class _MakePaymentView extends StatelessWidget {
  final BookingDetailsContent bookingDetails;
  final double totalBookingAmount;
  const _MakePaymentView(this.bookingDetails, {required this.totalBookingAmount});
        ...
        Expanded(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row( children: [
              ...
              Text( PriceConverter.convertPrice(totalBookingAmount),
                style: robotoMedium,
              ),
            ]),
