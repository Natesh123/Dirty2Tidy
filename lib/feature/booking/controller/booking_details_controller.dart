...
class BookingDetailsController extends GetxController implements GetxService{
      ...
      if(Get.find<UserController>().showReferWelcomeDialog() && Get.find<AuthController>().getIsShowReferralBottomSheet() == true){
        Future.delayed(const Duration(microseconds: 500), () {
          showModalBottomSheet(
            ...
            builder: (context) => ReferWelcomeDialog(),
            backgroundColor: Colors.transparent,
          );
        });
      }
  ...
  Future<void> downloadInvoice(BookingDetailsContent bookingDetailsContent) async {
    Get.dialog(const CustomLoader(), barrierDismissible: false);

    List<InvoiceItem> items = [];
    Set<String> seenItems = {}; // Deduplicate backend duplicate records

    bookingDetailsContent.bookingDetails?.forEach((element) {
      double cost = (element.serviceCost ?? 0).toDouble();
      int qty = element.quantity ?? 1;
      String key = "${element.variantKey ?? ''}_${cost}_${qty}";
      if (seenItems.contains(key)) return; // skip duplicate
      seenItems.add(key);

      items.add(InvoiceItem(
        discountAmount: element.discountAmount.toString(),
        tax: "0.0",
        serviceName: element.serviceName ?? "",
        variantKey: element.variantKey,
        quantity: qty,
        unitAllTotal: (cost * qty).toStringAsFixed(2),
        unitPrice: cost.toStringAsFixed(2),
      ));
    });

    Uint8List pdfBytes = await InvoiceController.generateUint8List(bookingDetailsContent, items, this);
