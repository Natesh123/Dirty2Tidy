...
class SubmitReviewController extends GetxController {
  ...
  void initializeReviewState(List<ItemService> bookingDetails) {
    for (var element in bookingDetails) {
      String? sId = element.serviceId ?? element.service?.id ?? element.id?.toString();
      if(sId != null) {
        if(!textControllers.containsKey(sId)) {
          textControllers[sId] = TextEditingController();
        }
        if(!selectedRating.containsKey(sId)) {
          selectedRating[sId] = 5;
        }
        if(!isEditable.containsKey(sId)) {
          isEditable[sId] = true;
        }
        if(!reviewComments.containsKey(sId)) {
          reviewComments[sId] = "";
        }
      }
    }
