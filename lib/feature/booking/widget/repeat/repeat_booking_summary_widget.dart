...
class RepeatBookingSummeryWidget extends StatelessWidget{
  ...
  Widget build(BuildContext context){
      return GetBuilder<BookingDetailsController>(builder:(bookingDetailsController){
       ...
       double baseSubTotal = minPrice + variantsTotal;
       double initialSubTotal = baseSubTotal * (bookingDetails.totalCount ?? 1);

       double updatedSubTotal = (bookingDetails.totalBookingAmount ?? 0) + totalDiscount + totalCouponDiscount + referralDiscountAmount - (extraFee + taxAmount);

       // Force Grand Total to match SubTotal (hiding VAT as requested)
       double totalBookingAmount = (initialSubTotal + extraFee) - (totalDiscount + totalCouponDiscount + referralDiscountAmount);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(
        ...
        Text(PriceConverter.convertPrice(BookingHelper.getBookingServiceUnitConst(bookingService, bookingDetails?.bookingDetails, bookingDetails!), isShowLongPrice:true,),
          style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeDefault,
              color: Theme.of(context).textTheme.bodyLarge!.color?.withValues(alpha: 0.9)
          ),
        ),
      ...
      _ServiceItemText(
        title: "unit_price".tr, 
        amount: (BookingHelper.getBookingServiceUnitConst(bookingService, bookingDetails?.bookingDetails, bookingDetails!) / (bookingService?.quantity ?? 1)), 
      ),
