...
class CheckoutHelper {
  ...
  static double calculateDiscount({required List<CartModel> cartList, required DiscountType discountType, int daysCount = 1 }){
    ...
    for (var cartModel in cartList) {
      if (cartModel.variantKey == 'base_service') continue;
  ...
  static double calculateSubTotal({required List<CartModel> cartList, int daysCount = 1}){
    ...
    for (var cartModel in cartList) {
      if (cartModel.variantKey != 'base_service') {
        subTotalPrice = subTotalPrice + ((cartModel.serviceCost * cartModel.quantity) * daysCount);
      }
      if (cartModel.service != null && cartModel.service!.id != null) {
        serviceIds.add(cartModel.service!.id!);
      }
    }
    for (var sid in serviceIds) {
      var s = cartList.firstWhere((element) => element.service?.id == sid).service;
      subTotalPrice += (s?.minBiddingPrice ?? 0) * daysCount;
    }
