...
class ServiceBookingController extends GetxController implements GetxService {
  ...
  List<BookingStatusTabs> get displayBookingStatusTabs => [
    BookingStatusTabs.all,
    BookingStatusTabs.accepted,
    BookingStatusTabs.ongoing,
    BookingStatusTabs.completed,
    BookingStatusTabs.canceled,
  ];
  ...
  Future<void> getAllBookingService({required int offset, required String bookingStatus, required bool isFromPagination, bool fromMenu = false, required String serviceType}) async {
    ...
    if (bookingStatus == "accepted") {
      Response responseAccepted = await serviceBookingRepo.getBookingList(offset: offset, bookingStatus: "accepted", serviceType: serviceType);
      Response responsePending = await serviceBookingRepo.getBookingList(offset: offset, bookingStatus: "pending", serviceType: serviceType);
      
      if (responseAccepted.statusCode == 200) {
        ServiceBookingList acceptedModel = ServiceBookingList.fromJson(responseAccepted.body);
        if (!isFromPagination) {
          _bookingList = [];
        }
        for (var element in acceptedModel.content!.bookingModel!) {
          _bookingList!.add(element);
        }
        
        if (responsePending.statusCode == 200) {
          ServiceBookingList pendingModel = ServiceBookingList.fromJson(responsePending.body);
          for (var element in pendingModel.content!.bookingModel!) {
            _bookingList!.add(element);
          }
        }
        ...
        int acceptedLastPage = responseAccepted.body['content']['last_page'] ?? 1;
        int pendingLastPage = (responsePending.statusCode == 200) ? (responsePending.body['content']['last_page'] ?? 1) : 1;
        _bookingListPageSize = acceptedLastPage > pendingLastPage ? acceptedLastPage : pendingLastPage;
        _bookingContent = acceptedModel.content!;
      } else {
        ApiChecker.checkApi(responseAccepted);
      }
    } else {
  ...
  Future<void> rescheduleBooking(String bookingId, String schedule) async {
    _isLoading = true;
    update();
    Response response = await serviceBookingRepo.updateBookingSchedule(bookingId: bookingId, schedule: schedule);
    _isLoading = false;
    update();
    if (response.statusCode == 200) {
      getAllBookingService(offset: 1, bookingStatus: _selectedBookingStatus.name.toLowerCase(), isFromPagination: false, serviceType: selectedServiceType.name);
  ...
  List<PopupMenuModel> getPopupMenuList({required String status, bool isRepeatBooking = false, RepeatBooking? ongoingRepeatBooking }) {
    if (status == "pending") {
      return [
        ...
        PopupMenuModel(title: "reschedule", icon: Icons.schedule),
