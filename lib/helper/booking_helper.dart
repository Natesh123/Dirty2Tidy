...
class BookingHelper {
  ...
  static double getSubTotalCost(BookingDetailsContent booking) {
    return (booking.totalBookingAmount ?? 0) 
        ...
        - (booking.totalTaxAmount ?? 0);
  }

  static double getBookingServiceUnitConst(ItemService? item, List<ItemService>? allItems, BookingDetailsContent booking) {
    return getCleanItemCost(item, booking);
  }

  static double getCleanItemCost(ItemService? item, BookingDetailsContent booking) {
    if (item?.serviceCost != null) {
      return (item!.serviceCost!) * (item.quantity ?? 1);
    }
    double totalCost = item?.totalCost ?? ((item?.serviceCost ?? 0) * (item?.quantity ?? 1));
    return totalCost;
  }
