...
class DataSyncRepo {
  ...
  Future<ApiResponseModel<T>> fetchData<T>(String uri, DataSourceEnum source, {dynamic body, ApiMethodType method = ApiMethodType.get, Map<String, String>? headers} ) async {
    try {
      return source == DataSourceEnum.client || _isACachesDisable() ? await _fetchFromClient<T>(uri, method: method, body: body, headers: headers) : await _fetchFromLocalCache<T>(uri);
    } catch (e) {
  ...
  Future<ApiResponseModel<T>> _fetchFromClient<T>(String uri, {dynamic body,ApiMethodType method = ApiMethodType.get, Map<String, String>? headers}) async {
    final response = await _fetchResponseFromClient(uri, body: body, method: method, headers: headers);
  ...
  Future<Response> _fetchResponseFromClient (String uri,{dynamic body, ApiMethodType method = ApiMethodType.get, Map<String, String>? headers}){
    if(method == ApiMethodType.get){
      return apiClient.getData(uri, headers: headers);
    }else{
      return apiClient.postData(uri, body, headers: headers);
    }
  }
