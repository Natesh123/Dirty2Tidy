...
import 'package:demandium/feature/category/widget/most_viewed_category_view.dart';
import 'package:demandium/feature/category/widget/popular_category_view.dart';
import 'package:demandium/feature/home/widget/about_us_view.dart';
import 'package:demandium/feature/home/widget/user_review_view.dart';
...
class HomeScreen extends StatefulWidget {
  ...
  static Future<void> loadData(bool reload, {int availableServiceCount = 1}) async {
    ...
    if(availableServiceCount==0){
      ...
      // Priority 1: Essential for initial UI render
      await Future.wait([
      ...
      // Priority 2: Non-blocking background calls
      Future.wait([
        ...
        Get.find<CartController>().getCartListFromServer(shouldUpdate: reload),
        if(Get.find<AuthController>().isLoggedIn())  Get.find<AuthController>().updateToken(),
      ]);

      /* Unused/Commented out features - removing to speed up load
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    // Force enabled to show services even if backend says 0 or address is null
        ...
        child: RefreshIndicator(
          onRefresh: () async {
            ...
            if(availableServiceCount > 0){
              ...
              if(Get.find<AuthController>().isLoggedIn()) Get.find<ConversationController>().getUnreadConversationCount();
            }else{
              await Get.find<BannerController>().getBannerList(true);
            }
          },
