...
import 'package:demandium/feature/category/view/category_service_form_screen.dart';
import 'package:demandium/feature/service/view/customer_details_screen.dart';
...
class RouteHelper {
  ...
  static const String categoryServiceForm = '/category-service-form';
  ...
  static String getCustomerDetailsRoute() => customerDetails;
  ...
  static String getCheckoutRoute(String page,String currentPage,String addressId, {bool? reload, String? token, String? amount} ) {
    String query = '$checkout?currentPage=$currentPage&addressID=$addressId&reload=$reload&token=$token';
    if(amount != null) {
      String encodedAmount = base64Url.encode(utf8.encode(amount));
      query += '&amount=$encodedAmount';
    }
    return query;
  }
  ...
  static String allServiceScreenRoute(String fromPage, {String campaignID = '', String categoryId = ''}) => '$allServiceScreen?fromPage=$fromPage&campaignID=$campaignID&categoryId=$categoryId';
  static String getCategoryServiceFormRoute(String categoryId, String categoryName, String categoryImage) {
    List<int> encodedName = utf8.encode(categoryName);
    String dataName = base64Encode(encodedName);
    
    List<int> encodedImage = utf8.encode(categoryImage);
    String dataImage = base64Encode(encodedImage);
    
    return '$categoryServiceForm?categoryId=$categoryId&categoryName=$dataName&categoryImage=$dataImage';
  }
  ...
  static List<GetPage> routes = [
    GetPage(
      name: initial,
      page: () => getRoute(const BottomNavScreen(pageIndex: 0, previousAddress: null, showServiceNotAvailableDialog: true,)),
    ),
    ...
    GetPage(name: customerDetails, page: () => const CustomerDetailsScreen()),
    ...
    GetPage(name: notification, page: () => getRoute(const NotificationScreen())),
      ...
      GetPage(name: checkout, page: () {
          ...
          String? amount;
          if(Get.parameters.containsKey('amount') && Get.parameters['amount'] != "null") {
            try {
              List<int> decode = base64Url.decode(Get.parameters['amount']!.replaceAll(' ', '+'));
              amount = utf8.decode(decode);
            } catch (e) {
              //
            }
          }
    ...
    GetPage(binding: ServiceBinding(),name: allServiceScreen, page:  ()=> getRoute(AllServiceView(fromPage: Get.parameters['fromPage'],campaignID: Get.parameters['campaignID'],categoryId: Get.parameters['categoryId'],)),),
    GetPage(binding: ServiceBinding(),name: categoryServiceForm, page: () {
      String categoryName = '';
      String categoryImage = '';
      try{
        List<int> decode = base64Decode(Get.parameters['categoryName'] ?? "");
        categoryName = utf8.decode(decode);
        
        List<int> decodeImg = base64Decode(Get.parameters['categoryImage'] ?? "");
        categoryImage = utf8.decode(decodeImg);
      }catch(e){
        //
      }
      return getRoute(CategoryServiceFormScreen(
        categoryId: Get.parameters['categoryId'] ?? "",
