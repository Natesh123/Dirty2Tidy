...
import 'package:demandium/feature/service/widget/promotional_video_player.dart';

class AllServiceView extends StatefulWidget {
  final String?fromPage;
  final String? campaignID;
  final String? categoryId;
  const AllServiceView({super.key, this.fromPage, this.campaignID, this.categoryId});
...
class _AllServiceViewState extends State<AllServiceView> {
  ...
  void initState() {
    ...
    // Initializing Service List logic here for better reliability
    if(widget.fromPage == 'all_service' || widget.fromPage == null){
      Get.find<ServiceController>().getAllServiceList(1, true, categoryId: widget.categoryId);
      if(widget.categoryId != null){
        print("DEBUG_TRACE: INITSTATE Calling getPromotionalVideo for ${widget.categoryId}");
        Get.find<ServiceController>().getPromotionalVideo(widget.categoryId!);
      }
    } else if(widget.categoryId != null) {
       // Also checking if we are just falling through to category page logic
       Get.find<ServiceController>().getAllServiceList(1, true, categoryId: widget.categoryId);
       print("DEBUG_TRACE: INITSTATE Calling getPromotionalVideo for ${widget.categoryId}");
       Get.find<ServiceController>().getPromotionalVideo(widget.categoryId!);
    }
  ...
  Widget _buildBody(String? fromPage,BuildContext context,ScrollController scrollController){
    if(fromPage == 'popular_services') {
      ...
      return GetBuilder<ServiceController>(
        ...
        return FooterBaseView(
          ...
          child: SizedBox(
            ...
            child: Column(
              children: [
                if(serviceController.promotionalVideo != null)
                  Padding(
                    padding: const EdgeInsets.only(top: Dimensions.paddingSizeDefault),
                    child: PromotionalVideoPlayer(
                      videoUrl: serviceController.promotionalVideo!['video_url'],
                      description: serviceController.promotionalVideo!['description'],
                    ),
                  ),
                ...
                PaginatedListView(
                  ...
                  onPaginate: (int offset) async => await serviceController.getAllServiceList(offset, false, categoryId: widget.categoryId),
                  itemView: ServiceViewVertical(
                    fromPage: fromPage,
                    service: serviceController.allService,
                    padding: EdgeInsets.symmetric(
                      horizontal: ResponsiveHelper.isDesktop(context) ? Dimensions.paddingSizeExtraSmall : Dimensions.paddingSizeSmall,
      ...
      print("DEBUG: AllServiceView Received ID: $categoryId");
      return GetBuilder<ServiceController>(
          initState: (state){
            Get.find<ServiceController>().getAllServiceList(1, true, categoryId: categoryId);
            if(categoryId != null){
               Get.find<ServiceController>().getPromotionalVideo(categoryId);
            }
          },
          builder: (serviceController) {
            return FooterBaseView(
